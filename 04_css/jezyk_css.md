...
CSS (ang. *Cascading Style Sheets*) - Kaskadowe Arkusze Stylów - tworzony i rozwijany od 1994 roku język służący do określania wyglądu stron HTML.
...
Podsumowując - aby formatować wygląd dokumentu HTML przy pomocy CSS trzeba w pliku HTML (w sekcji `<head>`) umieścić `<link rel="stylesheet" href="style.css />`, a poza tym stworzyć plik style.css (nazwa może być inna, np. zielonastrona.css, ale nazwa style.css jest powszechnie stosowana).
...
- Element li wewnątrz listy o identyfikatorze #mustsee ma mieć tło żółte:
